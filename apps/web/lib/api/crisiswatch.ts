...
export async function fetchVisualRows(): Promise<GeoStrategicQueryResult["rows"]> {
  const response = await fetch("/api/visual-rows", {
    method: "GET"
  });
  return parseJson<GeoStrategicQueryResult["rows"]>(response);
}

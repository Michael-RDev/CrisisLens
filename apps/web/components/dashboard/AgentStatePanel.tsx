    ...
    <article className="integration-card min-w-0 overflow-hidden rounded-2xl border border-[#2e4f63] bg-[#10202d] p-4">
      ...
      {!agentLoading && agentState ? (
        <>
          ...
          <p className="break-words">{agentState.narrative}</p>
        </>
      ) : null}
      ...
      <ul className="grid list-none gap-1.5 p-0">
        {(agentState?.riskDrivers?.length
          ...
          <li key={driver} className="rounded-lg border border-[#2f5064] px-2.5 py-2 text-sm break-words">
            {driver}
          </li>
        ))}
      </ul>
      ...
      <ul className="grid list-none gap-1.5 p-0">
        {(agentState?.recommendedActions?.length
          ...
          <li key={action} className="rounded-lg border border-[#2f5064] px-2.5 py-2 text-sm break-words">
            {action}
          </li>
        ))}
      </ul>
